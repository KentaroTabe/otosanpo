...
- **iOS と Android は同じ機能で配る。**
  「配布しているバージョンが機種によって違う状況はあるべきではない。
  機種ごとの問題が起こるようになってからで良い」(2026-09-02)。
  **配る前に、配布中の版との差分を確かめる**:
  `git diff <配布中のタグ> <配る枝> -- config/parameters.json`。
  判断待ちの変更(効果が未確認のもの)が混ざっていないかを見る。
  一度これを踏んだ: 聴き比べ待ちの音の変更が Android の APK にだけ入り、
  iOS のテスターと違う音になっていた(公開前に停止)
