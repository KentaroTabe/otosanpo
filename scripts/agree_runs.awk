...
#
# 検疫(HeadingQuarantine)が要求する「course が取れていて、かつ差が閾値の内側」の
# **連続区間**が、実際のログでどれだけ続いていたかを数える。
#
# 検疫は course が nil の標本で窓を捨てる。course は間欠的にしか取れないため、
# 「連続 regain_sec 秒」が現実に成立しうるのかを確かめるために用意した(2026-09-10)。
#
# limit: 差の閾値 [deg](既定 40 = distrust_deg)
# show:  この秒数以上の区間を個別に並べる(既定 5 = regain_sec)
#
