#!/usr/bin/env bash
# 使い方:
#   scripts/build_map.sh <地域pbf> <中心緯度> <中心経度>
# 例:
#   scripts/build_map.sh ~/Downloads/kanto-latest.osm.pbf 35.6812 139.7671
#
# 地域全体の OSM 抽出データ(.osm.pbf)から、中心の周囲 map_radius_m だけを切り出し、
...
#
# 事前準備:
#   1. brew install osmium-tool
#   2. 地域の pbf を入手(例: Geofabrik の日本 > 関東)。数百 MB
#
# 生成物を iPhone へ入れる:
...
if ! command -v osmium > /dev/null; then
  echo "osmium が見つかりません。次で入れてください:" >&2
  echo "  brew install osmium-tool" >&2
  exit 1
fi

if [ $# -lt 3 ]; then
  echo "使い方: scripts/build_map.sh <地域pbf> <中心緯度> <中心経度>" >&2
  exit 1
fi
...
if [ ! -f "$PBF" ]; then
  echo "pbf が見つかりません: $PBF" >&2
  echo "Geofabrik などから地域の .osm.pbf を入手してください。" >&2
  exit 1
fi
