...
位置情報・ヘッドフォンモーション・音声出力の動作確認は **実機 + AirPods** でのみ可能。シミュレータでは Features > Location(GPX)と、画面の「設定 > 案内音 > 試聴」で代替する。実機で確認していない挙動は「未確認」と明示して報告する。

**「頭の追従の確認(机上)」と「時間到来を発火」は 2026-09-17 に削除した**(利用者判断)。時間到来の流れを画面から起こす手段はもう無い。
