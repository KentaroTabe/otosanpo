...
**「スポットを移す提案」(`SpotMoveSchedule`)は移していない。**
うなずきで受ける仕様なので、Android では受け口を決める必要がある(→ 判断待ち 7)。
...
| 7 | スポットを移す提案の受け口(2026-09-19) | **音量↓で移す・音量↑で断る(推奨。「帰る / 延長」と同じ流儀)** / Android では出さない(いまはこれ) / 画面のボタンだけ |
| 8 | 「真横に聞こえる角度」を Android にも置くか(2026-09-19) | **置かない(いまはこれ)** — あれは HRTF へ渡す角度を歪める設定で、等電力パンには効き方が違う / 置く(パンの係数として使う・別途の実測が要る) |
