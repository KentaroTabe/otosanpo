...
2. APK を作る。**相手の都市が分かっているなら、経路データごと同梱する**

```bash
scripts/build_android.sh 金沢市
```

   `dist/otosanpo-android-金沢市.apk` ができる(約 6.5 MB)。
   都市を渡さなければ `dist/otosanpo-android.apk`(約 3.3 MB・経路データなし)。

   > **同梱するようにしたのは、テスターが `Android/data/dev.otosanpo/files/` へ
   > ファイルを置けなかったため**(2026-08-28)。この場所は Android 11 以降、
   > 標準のファイルアプリから辿りにくく、機種によっては見えない。
   >
   > **端末に置かれたファイルがあればそちらを優先する**ので、
   > 自分で用意した人の地図を上書きしない。
   >
   > **毎回クリーンしてから作る。** 差分パッケージングは外した資産のバイト列を
   > APK に残す。金沢市を同梱したあと同梱なしで作り直すと、中央ディレクトリからは
   > 消えるのにファイルは 6.8 MB のままだった(クリーン後は 3.3 MB)。
   > **渡すつもりのない地図が混入する**ので、配布物では速さより確実さを取る

3. **GitHub Release の下書きを作る**(2026-09-02 から この形)

```bash
scripts/release_android.sh android-20260919 "Android テスター版 (2026-09-19)" notes.md
```

   3 つの APK(地図なし・名古屋市・金沢市)を作り、`dist/release/` へ
   **ローマ字名で**複製してから下書きを作る。

   - **アセット名はローマ字。** 日本語のファイル名は環境によって
     ダウンロード後に文字化けしたり、URL が読めなくなったりする
   - **タグはいま居る枝に付ける。** 既定のままだと main にタグが付き、
     **この APK を作ったコードと結びつかない**
   - **下書きで止める。** リポジトリは公開なので Release も公開になる。
     公開は人が GitHub の画面から行う

   リリースノートには**今回何を見てほしいか**を書く。iOS の TestFlight と違って
   「テスト内容」の欄が無いので、ノートが唯一の伝え口。
   1 回に頼むのは 1〜2 個まで(docs/05 の方針)

4. 公開し、リンクをテスターに渡す。**docs/11 はノートからリンクする**
...
| `Storage` | 設定・地図・自宅・履歴・速度・フィールドログ・**曲の取り込み** |
| `WalkService` | 常駐サービス(位置と音を画面消灯中も続ける) |
| `MusicPlayer` | 音楽スポットの曲(`MediaPlayer` + 等電力パン)。iOS の `EarconSynth.startMusic` に対応 |
| `MainActivity` | 出発前の画面。**音量ボタンで「帰る / 延長」** |
| `MenuActivity` | 左上の三本線から開く。試聴・ログ・出典・デバッグ。iOS の `MenuView` に対応 |
| `LogActivity` | フィールドログ / イベントログ(`EXTRA_MODE` で切り替え) |
| `Views` | 素の View を組む小道具(見出し・本文・ボタン・三本線の絵) |

### 画面の分け方(2026-09-19 利用者依頼・iOS と揃えた)

> 「毎回設定するわけではないものを格納しておくため左上にいわゆるハンバーガー
> (よこせん3本のメニューバー)を作り」

- **主画面**: (問いかけ中のみ)時間になりました / 自宅 / 散歩時間 / 音楽スポット /
  状態 / 開始 / 前回の散歩
- **メニュー**: 案内音の試聴 / フィールドログ / イベントログ / クレジット

**時間到来の応答は一番上・問いかけの間だけ**(2026-09-19 に iOS へ揃えた)。
常に並べておくと、押せない時に押せるように見える。
残りの延長回数もボタンに出す(「押しても延びない」を防ぐ)。

**「時間到来を発火」は置かない。** iOS 版では 2026-09-17 に消してある(CLAUDE.md)。
片方だけ画面から流れを起こせると、**同じ手順で試したつもりの記録が食い違う。**
`WalkSession.debugTimeUp` ごと消したので、時間到来は時計から来るものだけになった。

三本線の絵は枠組みの drawable に無いので、`ShapeDrawable` を 3 つ重ねて描く
(`Views.menuIcon`)。**画像を足すと `res/` を持つことになり、
「素の View だけ」の構成が崩れる。**

**上の帯(ActionBar)が無い端末では、画面の中に「≡ メニュー」を出す**
(`installMenuButton` が false を返した時)。メニューへ行けないと、
テスターがフィールドログを取り出せなくなる。

### 音楽スポット(2026-09-19 に移植)

**頭の向きを使わない形で移した**(利用者依頼)。基準は進む向きだけ。

| 手がかり | iOS | Android |
|---|---|---|
| 左右 | HRTF(`AVAudioEnvironmentNode`) | **等電力パン**(`MediaPlayer.setVolume(l, r)`) |
| 距離 | 音量 | **音量**(同じ計算) |
| 向きの遊び | `BearingHold` | **同じ**(Core を共有) |
| 前後(高域シェルフ) | `AVAudioUnitEQ` | 無い |
| 仰角(近づくと下から)・広がり | あり | **無い** |

前後はもともと伝わらないと実測で確定している(docs/03)ので、
失うのは**仰角と広がり**だけ。ここは Android の弱点として記録しておく。

**曲は `ACTION_OPEN_DOCUMENT` で選び、その場でアプリの中へ写す。**
`content://` を持ち続けない — 元を消された・SD を外された・提供元アプリを消された、
で失効するうえ、クラウド上の曲だと読むたびに通信が要る(散歩中に止まる)。
読める拡張子は `config/parameters.json` の `audio.music_source_extensions` のうち、
Android が標準で復号できるものに限る(`AppParameters.androidReadableExtensions`)。

#### スポットを移す提案(2026-09-19 利用者判断)

`SpotMoveSchedule` も移した。**時刻の勘定は iOS とまったく同じ**で、違うのは
**応答の受け口だけ**。
...
「帰る / 延長」と同じ流儀にした(利用者判断)。ポケットの中でも押せる。
**2 つの問いかけが同時に開くことはない** — 移す提案は散策中しか出さない。

**中断の扱いだけ iOS と違う。** 帰路の問いかけが割り込んで応答の窓が残った場合、
Android は `postpone`(断りに数えない)、iOS は窓が期限切れになって `refused`
(断りに数える)になる。**仕様(合議 E10)に合っているのは Android 側。**
iOS は配布中の 1.1 に入っているので、次の版で揃える(→ docs/08)。
...
| 5 | テスト端末 | **手元に Android 実機は無い**(2026-08-26 に確認)。持っている知人に頼む前提で進める。段 5(実機テスト)はその人の都合次第。エミュレータは入れたが**ディスクの空きが足りず起動できていない**(下記) |
| 6 | Play ストアに出すか | **APK を手渡しのまま(いまはこれ)** / ストアに出す(初回登録料が要り、配布用の署名鍵も作ることになる) |
## エミュレータ(2026-09-19 に用意・**画面を初めて確かめた**)

実機が無いので、せめて画面の遷移だけでも見るために入れた。

- `scripts/setup_android_emulator.sh` — `emulator` と
  `system-images;android-35;google_apis;arm64-v8a` を入れ、AVD `otosanpo-test` を作る
- `scripts/run_android_emulator.sh` — 起動 → APK を入れる → 許可を与える → 開く。
  `shot <名前>` で `dist/shots/<名前>.png` に画面を撮る

**確かめられるのは画面の遷移だけ。** GPS の質・音の左右・歩調は実機でしか分からない。

### これで見つかった不具合 2 件(**どちらもコンパイルは通っていた**)

| 見つかったもの | 原因 |
|---|---|
| **三本線が黒い四角**になる | `LayerDrawable.setLayerInset` は子の固有サイズに**加算**される。枠が 24dp を超えて 3 本が太り、潰れていた → `Canvas` で直に描く形に変えた |
| **主画面の上端が隠れる** | `targetSdk 35` は Android 15 から**端から端まで描く**のが既定。素の Activity では内容が窓の y=0 から並び、状態バーと帯が覆う → 差し込まれた余白を `ScrollView` に当てた |

2 件目は**テスターが最初に押す「自宅を現在地に設定」が 40px しか見えていなかった。**
実測値: `ScrollView` の bounds が `[0,0][1080,2400]`、「設定」が y 32–134、
「自宅: 未設定」が 134–196(どちらも帯の下)。

余白は**自分で帯の高さを足さない**。内容へ配られる余白には既に入っている
(実測 275px = 状態バー 132 + 帯 147)。足したら帯と「設定」の間が 150px 空いた。

### 確かめた道(2026-09-19)

主画面 → 三本線 → メニュー → フィールドログ → 戻る → 曲を選ぶ(SAF)→
Downloads → 選択 → 取り込み → 音楽スポットを入にする。**すべて通った。**

取り込みは `files/music-source.wav`(264,644 バイト = 置いた WAV と同じ)。
**画面には `music-source.wav` と出る**(元の名前ではない)。iOS も同じ規則
(`MusicStore.chosenBase`)なので、そろえたまま置いてある。

### 踏んだこと(記録)

**ディスクの空きが 7.4 GB 要る。** エミュレータは userdata に
6 GB の 1.2 倍を要求し、**設定では下げられない:**

| 試したこと | 結果 |
|---|---|
| `disk.dataPartition.size` を 2G に | 要求は 7372.80 MB のまま。エミュレータが 6 GB へ書き戻す |
| 同じく 3G に | 同じ |
| `-partition-size 3072` | 同じ(この旗は userdata の検査に効かない) |

両スクリプトは**起動前に空きを見て、足りなければ必要な GB 数を言って止まる**。

**GPU は `swiftshader_indirect`(ソフトウェア描画)にする。**

| 設定 | 画面を撮れるか | SystemUI |
|---|---|---|
| `-gpu auto`(Mac では Metal) | **真っ黒**。窓なしでは面を読めない | 軽い |
| `-gpu swiftshader_indirect` | **撮れる** | 重い |

撮れないと確かめる意味が無いのでソフトウェア描画を採り、
**アニメーションを切って**(`window_animation_scale` 等を 0)SystemUI の負荷を下げる。
それでも描画は 1 fps 前後なので、**押してから画面が変わるまで数十秒かかる。**
`adb shell dumpsys activity activities` で前面の Activity を見て待つこと。

## 決まったこと(2026-09-19 利用者判断)

| 項目 | 決定 |
|---|---|
| スポットを移す提案の受け口 | **音量↓で移す・音量↑でそのまま。**「帰る / 延長」と同じ流儀にする |
| 「真横に聞こえる角度」を Android にも置くか | **置かない。** HRTF へ渡す角度を歪める設定で、等電力パンには効き方が違う。Core には移してあるので、実測して効き方が分かれば足せる |
| アナリティクス | **今回は入れない。** 計測はフィールドログ(端末内の TSV)のままにする |
| 版番号 | iOS と揃えて 1.1 |
